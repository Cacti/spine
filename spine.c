...
int main(int argc, char *argv[]) {
	...
	qp += snprintf(qp, MEGA_BUFSIZE - (qp - querybuf), "SELECT SQL_NO_CACHE id, device_threads, picount, picount/device_threads AS tppi FROM host AS h LEFT JOIN (SELECT host_id, COUNT(*) AS picount FROM poller_item GROUP BY host_id) AS pi ON h.id = pi.host_id");
	qp += snprintf(qp, MEGA_BUFSIZE - (qp - querybuf), " WHERE disabled = ''");

	qp += snprintf(qp, MEGA_BUFSIZE - (qp - querybuf), " AND availability_method != %d", AVAIL_STREAM);

	if (!strlen(set.host_id_list)) {
		qp += append_hostrange(qp, "h.id");	/* AND id BETWEEN a AND b */
	} else {
		qp += snprintf(qp, MEGA_BUFSIZE - (qp - querybuf), " AND h.id IN(%s)", set.host_id_list);
	}

	qp += snprintf(qp, MEGA_BUFSIZE - (qp - querybuf), " AND h.poller_id = %i", set.poller_id);
	qp += snprintf(qp, MEGA_BUFSIZE - (qp - querybuf), " ORDER BY picount DESC");
