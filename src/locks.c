...
DEFINE_SPINE_LOCK(spine_host_time)
...
void init_mutexes(void) {
	...
	pthread_once((pthread_once_t*) get_attr(LOCK_HOST_TIME_O),   init_spine_host_time_lock);
}
...
static const char *get_name(int lock) {
	switch (lock) {
		...
		case LOCK_HOST_TIME:   return "spine_host_time";
	}
...
pthread_cond_t* get_cond(int lock) {
	...
	switch (lock) {
		...
		case LOCK_HOST_TIME:   ret_val = &spine_host_time_cond;   break;
	}
...
pthread_mutex_t* get_lock(int lock) {
	...
	switch (lock) {
		...
		case LOCK_HOST_TIME:   ret_val = &spine_host_time_lock;   break;
	}
...
pthread_once_t* get_attr(int locko) {
	...
	switch (locko) {
		...
		case LOCK_HOST_TIME_O:   ret_val = &spine_host_time_lock_o;   break;
	}
