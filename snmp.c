...
netsnmp_ds_toggle_boolean(NETSNMP_DS_LIBRARY_ID, NETSNMP_DS_LIB_DONT_CHECK_RANGE);
	...
	SPINE_LOG_DEBUG("SNMP Header Version is %s", PACKAGE_VERSION);
	SPINE_LOG_DEBUG("SNMP Library Version is %s", netsnmp_get_version());
...
#else
	SPINE_LOG_DEBUG("Issues with SNMP Header Version information, assuming old version of Net-SNMP.");
	init_snmp("spine");
#endif
}
...
void *snmp_host_init(int host_id, char *hostname, int snmp_version, char *snmp_community,
	...
	if (!sessp) {
		if (is_debug_device(host_id)) {
			SPINE_LOG("Device[%i] Problem initializing SNMP session '%s'", host_id, hostname);
		} else {
			SPINE_LOG_MEDIUM("Device[%i] Problem initializing SNMP session '%s'", host_id, hostname);
		}
	}
...
char *snmp_get_base(host_t *current_host, char *snmp_oid, bool should_fail) {
	...
	if (current_host->ignore_host) {
		SPINE_LOG_HIGH("Skipped oid '%s' for Device[%i] as host ignore flag is active", snmp_oid, current_host->id);
	...
	if (current_host->snmp_session != NULL) {
		...
		if (status == STAT_DESCRIP_ERROR) {
			SPINE_LOG("Unable to create SNMP PDU");
			...
			if (response == NULL) {
				SPINE_LOG("An internal Net-Snmp error condition detected in Cacti snmp_get");
				...
				if (vars->type == SNMP_NOSUCHOBJECT) {
					if (!strstr(snmp_oid, ".1.3.6.1.2.1.1.1.0") && !strstr(snmp_oid, ".1.3.6.1.2.1.1.3.0")) {
						SET_UNDEFINED(result_string);
						status = STAT_ERROR;
						SPINE_LOG_DEBUG("OID '%s' for Device[%i], SNMP_NOSUCHOBJECT not sysDesc or sysUptime", snmp_oid, current_host->id);
					} else {
						SPINE_LOG_HIGH("OID '%s' for Device[%i], SNMP_NOSUCHOBJECT for sysDesc or sysUptime", snmp_oid, current_host->id);
					...
					if (!strstr(snmp_oid, ".1.3.6.1.6.3.10.2.1.3.0")) {
						SPINE_LOG_HIGH("No such Instance for oid '%s' for Device[%i] with Status[%d]",  snmp_oid, current_host->id, status);
					} else {
						SPINE_LOG_DEBUG("Legacy SNMP agent found!  No per second Uptime oid found '%s' for Device[%i] with Status[%d]",  snmp_oid, current_host->id, status);
					}
				} else if (vars->type == SNMP_ENDOFMIBVIEW) {
					SET_UNDEFINED(result_string);
					status = STAT_ERROR;
					SPINE_LOG_HIGH("End of Mib for oid '%s' for Device[%i] with Status[%d]",  snmp_oid, current_host->id, status);
				} else {
					snprint_value(temp_result, RESULTS_BUFFER, vars->name, vars->name_length, vars);
				...
				SPINE_LOG_HIGH("Failed to get oid '%s' for Device[%i] with Response[%ld]",  snmp_oid, current_host->id, response->errstat);
			}
			...
			if (vars->type == SNMP_NOSUCHOBJECT) {
				SET_UNDEFINED(result_string);
				status = STAT_ERROR;
				SPINE_LOG_HIGH("No such Object for oid '%s' for Device[%i] with Status[%d]",  snmp_oid, current_host->id, status);
			} else if (vars->type == SNMP_NOSUCHINSTANCE) {
				SET_UNDEFINED(result_string);
				status = STAT_ERROR;
				...
				if (!strstr(snmp_oid, ".1.3.6.1.6.3.10.2.1.3.0")) {
					...
					SPINE_LOG_DEBUG("Per second level uptime oid missing oid '%s' for Device[%i] with Status[%d]",  snmp_oid, current_host->id, status);
				}
			} else if (vars->type == SNMP_ENDOFMIBVIEW) {
				SET_UNDEFINED(result_string);
				status = STAT_ERROR;
				...
				SET_UNDEFINED(result_string);
				status = STAT_ERROR;
				SPINE_LOG_HIGH("Unknown error getting oid '%s' for Device[%i] with Status[%d]",  snmp_oid, current_host->id, status);
			}
		} else if (status == STAT_TIMEOUT) {
			SPINE_LOG_HIGH("Timeout getting oid '%s' for Device[%i] with Status[%d]",  snmp_oid, current_host->id, status);
		} else {
...
#endif
					SPINE_LOG_MEDIUM("Device[%i] USM notInTimeWindow for oid '%s' -- engine time drift (recoverable)",
...
#endif
					SPINE_LOG_HIGH("Device[%i] USM unknownEngineID for oid '%s'",
						current_host->id, snmp_oid);
					break;
#endif
...
#endif
					SPINE_LOG_HIGH("Device[%i] USM unknownSecurityName for oid '%s'",
						current_host->id, snmp_oid);
					break;
#endif
...
#endif
					SPINE_LOG_HIGH("Device[%i] USM authenticationFailure for oid '%s'",
						current_host->id, snmp_oid);
					break;
#endif
...
#endif
					SPINE_LOG_HIGH("Device[%i] USM decryptionError for oid '%s'",
						current_host->id, snmp_oid);
					break;
#endif
...
#endif
					SPINE_LOG_HIGH("Device[%i] USM unsupportedSecurityLevel for oid '%s'",
						current_host->id, snmp_oid);
					break;
#endif
				default:
					SPINE_LOG_HIGH("Unknown error getting oid '%s' for Device[%i] with Status[%d] Errno[%d]",
...
char *snmp_getnext(host_t *current_host, char *snmp_oid) {
	...
	if (current_host->snmp_session != NULL) {
		...
		if (status == STAT_SUCCESS) {
			if (response == NULL) {
				SPINE_LOG("An internal Net-Snmp error condition detected in Cacti snmp_getnext");
			...
			switch (sess_liberr) {
				...
				case SNMPERR_USM_NOTINTIMEWINDOW:
#endif
					SPINE_LOG_MEDIUM("Device[%i] USM notInTimeWindow for getnext oid '%s' -- engine time drift (recoverable)",
						current_host->id, snmp_oid);
					/* treat as transient, do not mark host down */
					status = STAT_SUCCESS;
					SET_UNDEFINED(result_string);
				...
				case SNMPERR_USM_UNKNOWNENGINEID:
#endif
					SPINE_LOG_HIGH("Device[%i] USM unknownEngineID for getnext oid '%s'",
						current_host->id, snmp_oid);
					break;
#endif
#if defined(SNMPERR_UNKNOWN_USER_NAME) || defined(SNMPERR_USM_UNKNOWNSECURITYNAME)
				...
				case SNMPERR_USM_UNKNOWNSECURITYNAME:
#endif
					SPINE_LOG_HIGH("Device[%i] USM unknownSecurityName for getnext oid '%s'",
						current_host->id, snmp_oid);
					break;
#endif
#if defined(SNMPERR_AUTHENTICATION_FAILURE) || defined(SNMPERR_USM_AUTHENTICATIONFAILURE)
				...
				case SNMPERR_USM_AUTHENTICATIONFAILURE:
#endif
					SPINE_LOG_HIGH("Device[%i] USM authenticationFailure for getnext oid '%s'",
						current_host->id, snmp_oid);
					break;
#endif
#if defined(SNMPERR_DECRYPTION_ERR) || defined(SNMPERR_USM_DECRYPTIONERROR)
				...
				case SNMPERR_USM_DECRYPTIONERROR:
#endif
					SPINE_LOG_HIGH("Device[%i] USM decryptionError for getnext oid '%s'",
						current_host->id, snmp_oid);
					break;
#endif
#if defined(SNMPERR_UNSUPPORTED_SEC_LEVEL) || defined(SNMPERR_USM_UNSUPPORTEDSECURITYLEVEL)
				...
				case SNMPERR_USM_UNSUPPORTEDSECURITYLEVEL:
#endif
					SPINE_LOG_HIGH("Device[%i] USM unsupportedSecurityLevel for getnext oid '%s'",
						current_host->id, snmp_oid);
					break;
#endif
				default:
...
int snmp_count(host_t *current_host, char *snmp_oid) {
	...
	if (is_debug_device(current_host->id)) {
		SPINE_LOG("walk starts at OID %s", snmp_oid);
	} else {
		SPINE_LOG_DEBUG("walk starts at OID %s", snmp_oid);
	}
		...
		while (ok && !error_occurred) {
			...
			if (status == STAT_SUCCESS) {
				if (response->errstat == SNMP_ERR_NOERROR) {
					...
					SPINE_LOG("An internal Net-Snmp error condition detected in Cacti snmp_count");
				}
			} else if (status == STAT_TIMEOUT) {
				SPINE_LOG("Timeout detected in Cacti snmp_count");
				ok = 0;
				error_occurred = 1;
			} else { /* status == STAT_ERROR */
				SPINE_LOG("An internal Net-Snmp error condition detected in Cacti snmp_count (STAT_ERROR)");
				ok = 0;
				error_occurred = 1;
			}
...
void snmp_snprint_value(char *obuf, size_t buf_len, const oid *objid, size_t objidlen, struct variable_list *variable) {
	...
	if (buf_len > 0) {
		CALLOC_OR_DIE(buf, u_char, buf_len, 1, "snmp.c buf");
...
void snmp_get_multi(host_t *current_host, target_t *poller_items, snmp_oids_t *snmp_oids, int num_oids) {
	...
	if (status == STAT_SUCCESS) {
		if (response == NULL) {
			SPINE_LOG("An internal Net-Snmp error condition detected in Cacti snmp_get_multi");
			status = STAT_ERROR;
		} else {
			if (response->errstat == SNMP_ERR_NOERROR) {
				vars = response->variables;
