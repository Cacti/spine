		...
		if (set.SQL_readonly == FALSE) {
			if (mysql_query(mysql, query)) {
				...
				if ((error == 1213) || (error == 1205)) {
					...
					if (error_count > 30) {
						SPINE_LOG("Too many Lock/Deadlock errors occurred!, SQL Fragment:'%s'", query_frag);
						return FALSE;
					}

					continue;
				} else {
					SPINE_LOG("SQL Failed! Error:'%i', Message:'%s', SQL Fragment:'%s'", error, mysql_error(mysql), query_frag);
					return FALSE;
				}
			} else {
				return TRUE;
...
int db_reconnect(MYSQL *mysql, int type, int error, char *function) {
	...
	if (mysql_thread_id(mysql) != mysql_thread) {
		SPINE_LOG("Connection Broken in Function %s with Error %i.  Reconnect via mysql_ping() successful.", function, error);
	...
	SPINE_LOG("Connection Broken in Function %s with Error %i.  Attempting explicit reconnect.", function, error);

	mysql_close(mysql);
	db_connect(type, mysql);

	if (mysql_thread_id(mysql) > 0) {
		SPINE_LOG("Explicit reconnect successful in Function %s.", function);
		return TRUE;
	}

	SPINE_LOG("Connection Broken with Error %i.  Reconnect failed.", error);
	return FALSE;
}
	...
	while (1) {
		if (mysql_query(mysql, query)) {
			...
			if (error == 2013 || error == 2006) {
				...
				SPINE_LOG("Database Error:'%i', Message:'%s'", error, mysql_error(mysql));
				SPINE_LOG("The Query Was:'%s'", query);
				exit(1);
			}
		} else {
...
void db_connect(int type, MYSQL *mysql) {
	...
	SPINE_LOG_DEBUG("Total Connections made %i", connections);
...
void db_create_connection_pool(int type) {
	...
	if (type == LOCAL) {
		SPINE_LOG_DEBUG("Creating Local Connection Pool of %i threads.", set.threads);

		for(id = 0; id < set.threads; id++) {
			SPINE_LOG_DEBUG("Creating Local Connection %i.", id);
		...
		SPINE_LOG_DEBUG("Creating Remote Connection Pool of %i threads.", set.threads);

		for(id = 0; id < set.threads; id++) {
			SPINE_LOG_DEBUG("Creating Remote Connection %i.", id);
...
void db_close_connection_pool(int type) {
	...
	if (type == LOCAL) {
		for(id = 0; id < set.threads; id++) {
			SPINE_LOG_DEBUG("Closing Local Connection Pool ID %i", id);
		...
		for(id = 0; id < set.threads; id++) {
			SPINE_LOG_DEBUG("Closing Remote Connection Pool ID %i", id);
...
pool_t *db_get_connection(int type) {
	...
	if (type == LOCAL) {
		SPINE_LOG_DEBUG("Traversing Local Connection Pool for free connection.");
		for (id = 0; id < set.threads; id++) {
			SPINE_LOG_DEBUG("Checking Local Pool ID %i.", id);
			if (db_pool_local[id].free == TRUE) {
				SPINE_LOG_DEBUG("Allocating Local Pool ID %i.", id);
		...
		SPINE_LOG_DEBUG("Traversing Remote Connection Pool for free connection.");
		for (id = 0; id < set.threads; id++) {
			SPINE_LOG_DEBUG("Checking Remote Pool ID %i.", id);
			if (db_pool_remote[id].free == TRUE) {
				SPINE_LOG_DEBUG("Allocating Remote Pool ID %i.", id);
	...
	SPINE_LOG("Connection Pool Fatal Error.");
...
void db_release_connection(int type, int id) {
	...
	if (type == LOCAL) {
		SPINE_LOG_DEBUG("Freeing Local Pool ID %i", id);
		db_pool_local[id].free = TRUE;
	} else {
		SPINE_LOG_DEBUG("Freeing Remote Pool ID %i", id);
		db_pool_remote[id].free = TRUE;
	}
...
int sql_buffer_init(sql_buffer_t *sb, size_t initial_capacity) {
	if (sb == NULL || initial_capacity == 0 || initial_capacity > SQL_MAX_BUFFER_CAPACITY) {
		SPINE_LOG("sql_buffer_init invalid capacity request '%zu' (max '%d')",
	...
	if (sb->buffer == NULL) {
		SPINE_LOG("sql_buffer_init failed to allocate '%zu' bytes", initial_capacity);
		sb->capacity = 0;
		sb->length   = 0;
		return -1;
	}
...
int sql_buffer_append(sql_buffer_t *sb, const char *format, ...) {
	...
	if (sb == NULL || sb->buffer == NULL || format == NULL) {
		SPINE_LOG("sql_buffer_append called with invalid arguments");
		return -1;
	}

	if (sb->length >= sb->capacity) {
		SPINE_LOG("sql_buffer_append detected invalid state (length '%zu' >= capacity '%zu')",
	...
	if (written < 0) {
		...
		SPINE_LOG("sql_buffer_append failed during format operation");
		va_end(args_copy);
		return -1;
	}
	...
	if ((size_t)written >= available) {
		...
		if (required_capacity > SQL_MAX_BUFFER_CAPACITY) {
			sb->buffer[sb->length] = '\0';
			SPINE_LOG("sql_buffer_append exceeded SQL_MAX_BUFFER_CAPACITY '%d' (required '%zu')",
		...
		if (new_capacity < required_capacity) {
			sb->buffer[sb->length] = '\0';
			SPINE_LOG("sql_buffer_append could not reach required capacity '%zu' (max '%d')",
		...
		if (new_buffer == NULL) {
			sb->buffer[sb->length] = '\0';
			SPINE_LOG("sql_buffer_append failed to reallocate buffer to '%zu' bytes", new_capacity);
			va_end(args_copy);
			return -1;
		}
		...
		if (written < 0 || (size_t)written >= (sb->capacity - sb->length)) {
			sb->buffer[sb->length] = '\0';
			SPINE_LOG("sql_buffer_append failed after reallocation");
			va_end(args_copy);
			return -1;
		}
	}
