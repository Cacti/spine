...
void read_config_options() {
	...
	/* check for version mismatch between Spine and Cacti */
	{
		...
		if (set.cacti_version != spine_version) {
			SPINE_LOG("Spine Version Mismatch! Spine Version:'%s', Cacti Version:'%d.%d.%d'", VERSION, (set.cacti_version / 1000), (set.cacti_version % 1000) / 100, (set.cacti_version % 100));
		}
	}

	/* log the path_webroot variable */
	SPINE_LOG_DEBUG("The binary Cacti version is %d", set.cacti_version);
	...
	/* log the path_webroot variable */
	SPINE_LOG_DEBUG("The path_php_server variable is %s", set.path_php_server);

	/* log the path_cactilog variable */
	SPINE_LOG_DEBUG("The path_cactilog variable is %s", set.path_logfile);

	/* the version variable */
	SPINE_LOG_DEBUG("The version variable is %s", set.dbversion);

	/* log the log_destination variable */
	SPINE_LOG_DEBUG("The log_destination variable is %i (%s)",
		set.log_destination,
	...
	SPINE_LOG_DEBUG("The path_php variable is %s", set.path_php);
	...
	SPINE_LOG_DEBUG("The availability_method variable is %i", set.availability_method);
	...
	SPINE_LOG_DEBUG("The ping_recovery_count variable is %i", set.ping_recovery_count);
	...
	SPINE_LOG_DEBUG("The ping_failure_count variable is %i", set.ping_failure_count);
	...
	SPINE_LOG_DEBUG("The ping_method variable is %i", set.ping_method);
	...
	SPINE_LOG_DEBUG("The ping_retries variable is %i", set.ping_retries);
	...
	SPINE_LOG_DEBUG("The ping_timeout variable is %i", set.ping_timeout);
	...
	SPINE_LOG_DEBUG("The boost_redirect variable is %i", set.boost_redirect);
	...
	SPINE_LOG_DEBUG("The boost_rrd_update_enable variable is %i", set.boost_enabled);

	/* set logging option for statistics */
	set.log_pstats = getboolsetting(&mysql, LOCAL, "log_pstats", FALSE);

	/* log the log_pstats variable */
	SPINE_LOG_DEBUG("The log_pstats variable is %i", set.log_pstats);
	...
	SPINE_LOG_DEBUG("The threads variable is %i", set.threads);
	...
	if (set.poller_interval == 0) {
		SPINE_LOG_DEBUG("The polling interval is the system default");
	} else {
		SPINE_LOG_DEBUG("The polling interval is %i seconds", set.poller_interval);
	}
	...
	SPINE_LOG_DEBUG("The number of concurrent processes is %i", set.num_parent_processes);
	...
	SPINE_LOG_DEBUG("The script timeout is %i", set.script_timeout);
	...
	SPINE_LOG_DEBUG("The selective_device_debug variable is %s", set.selective_device_debug);
	...
	SPINE_LOG_DEBUG("The spine_log_level variable is %i", set.spine_log_level);
	...
	/* log the script timeout value */
	SPINE_LOG_DEBUG("The number of php script servers to run is %i", set.php_servers);
	...
	/* log the script timeout value */
	SPINE_LOG_DEBUG("The number of active data source profiles is %i", set.active_profiles);
	...
	/* log the script timeout value */
	SPINE_LOG_DEBUG("The number of snmp ports on the system is %i", set.total_snmp_ports);
	...
	if (!strlen(set.host_id_list)) {
		...
		SPINE_LOG_DEBUG("StartDevice='%i', EndDevice='%i', TotalPHPScripts='%i'",
			set.start_host_id,
			set.end_host_id,
			num_rows);
	} else {
		...
		SPINE_LOG_DEBUG("Device List to be polled='%s', TotalPHPScripts='%i'",
			set.host_id_list,
			num_rows);
	}

	SPINE_LOG_DEBUG("The PHP Script Server is %sRequired",
		set.php_required
		? ""
		: "Not ");
	...
	SPINE_LOG_DEBUG("The Maximum SNMP OID Get Size is %i", set.snmp_max_get_size);
...
static int flush_sql_batch(MYSQL *mysqlr, sql_buffer_t *sb, const char *suffix, const char *context) {
	...
	if (sql_buffer_append(sb, "%s", suffix) != 0) {
		SPINE_LOG("Failed to append SQL suffix while flushing '%s' batch", context);
		sql_buffer_reset(sb);
		return -1;
	}

	if (db_insert(mysqlr, REMOTE, sb->buffer) == FALSE) {
		SPINE_LOG("Failed to insert '%s' batch into remote database", context);
		sql_buffer_reset(sb);
		return -1;
	}
...
void poller_push_data_to_main() {
	...
	if (sql_buffer_init(&sb, HUGE_BUFSIZE) != 0) {
		SPINE_LOG("Could not initialize SQL buffer in poller_push_data_to_main");
		db_disconnect(&mysql);
		db_disconnect(&mysqlr);
		return;
	}
	...
	if ((result = db_query(&mysql, LOCAL, query)) != 0) {
			...
			while ((row = mysql_fetch_row(result))) {
				...
				if (rows >= 500) {
					if (flush_sql_batch(&mysqlr, &sb, suffix, "host-status") != 0) {
						SPINE_LOG("Failed to flush host-status batch before appending row id '%s'", row_id);
					}
				...
				if (append_host_status_row(&sb, &mysql, row, tmpstr, sizeof(tmpstr), prefix, rows > 0) != 0) {
					SPINE_LOG("SQL append failed for host-status row id '%s'; attempting flush+retry", row_id);

					if (rows > 0) {
						if (flush_sql_batch(&mysqlr, &sb, suffix, "host-status-retry") != 0) {
							SPINE_LOG("Flush during host-status retry failed for row id '%s'", row_id);
							rows = 0;
							continue;
						}
					...
					if (append_host_status_row(&sb, &mysql, row, tmpstr, sizeof(tmpstr), prefix, 0) != 0) {
						SPINE_LOG("Fatal host-status append failure after retry for row id '%s'; dropping row", row_id);
						continue;
					}
				}
		...
		if (rows > 0) {
			if (flush_sql_batch(&mysqlr, &sb, suffix, "host-status-final") != 0) {
				SPINE_LOG("Failed to flush final host-status batch");
			}
		}
	}
	...
	if ((result = db_query(&mysql, LOCAL, query)) != 0) {
			...
			while ((row = mysql_fetch_row(result))) {
				...
				if (rows >= 10000) {
					if (flush_sql_batch(&mysqlr, &sb, suffix, "poller-item") != 0) {
						SPINE_LOG("Failed to flush poller-item batch before appending row id '%s'", row_id);
					}
				...
				if (append_poller_item_row(&sb, &mysql, row, tmpstr, sizeof(tmpstr), prefix, rows > 0) != 0) {
					SPINE_LOG("SQL append failed for poller-item row id '%s'; attempting flush+retry", row_id);

					if (rows > 0) {
						if (flush_sql_batch(&mysqlr, &sb, suffix, "poller-item-retry") != 0) {
							SPINE_LOG("Flush during poller-item retry failed for row id '%s'", row_id);
							rows = 0;
							continue;
						}
					...
					if (append_poller_item_row(&sb, &mysql, row, tmpstr, sizeof(tmpstr), prefix, 0) != 0) {
						SPINE_LOG("Fatal poller-item append failure after retry for row id '%s'; dropping row", row_id);
						continue;
					}
				}
		...
		if (rows > 0) {
			if (flush_sql_batch(&mysqlr, &sb, suffix, "poller-item-final") != 0) {
				SPINE_LOG("Failed to flush final poller-item batch");
			}
		}
	}
...
int hasCaps() {
	...
	caps = cap_get_proc();
	if (caps == NULL) {
		SPINE_LOG("cap_get_proc failed.");
		return FALSE;
	}

    /* check if cap_net_raw is in effective set */
	if (cap_get_flag(caps, CAP_NET_RAW, CAP_EFFECTIVE, &capflag)) {
		SPINE_LOG("cap_get_flag for CAP_NET_RAW failed. ICMP ping will not work as non-root user.");
		return FALSE;
	}

	if (capflag != CAP_SET) {
		SPINE_LOG("Capability CAP_NET_RAW is not set. ICMP ping will not work as non-root user.");
		return FALSE;
	}

	SPINE_LOG_DEBUG("Capability CAP_NET_RAW is set.");
	cap_free(caps);
...
void checkAsRoot() {
	...
	if (privset == NULL) {
		...
		SPINE_LOG_DEBUG("Basic privset is: '%s'.", p != NULL ? p : "Unknown");
	}

	/* Add privilege to send/receive ICMP packets */
	if (priv_addset(privset, PRIV_NET_ICMPACCESS) < 0) {
		SPINE_LOG_DEBUG("Addition of PRIV_NET_ICMPACCESS to privset failed: '%s'.", strerror(errno));
	}
	...
	if (setppriv(PRIV_OFF, PRIV_PERMITTED, privset) < 0) {
		SPINE_LOG_DEBUG("Dropping privileges from PRIV_PERMITTED failed: '%s'.", strerror(errno));
	}

	/* Remove unneeded priv set from Limit to be safe */
	if (setppriv(PRIV_OFF, PRIV_LIMIT, privset) < 0) {
		SPINE_LOG_DEBUG("Dropping privileges from PRIV_LIMIT failed: '%s'.", strerror(errno));
	}

	boolean_t pe = priv_ineffect(PRIV_NET_ICMPACCESS);
	SPINE_LOG_DEBUG("Privilege PRIV_NET_ICMPACCESS is: '%s'.", pe != 0 ? "Enabled" : "Disabled");
	...
	if (hasCaps() != TRUE) {
		int ret;
		SPINE_LOG_DEBUG("Spine running as %d UID, %d EUID", getuid(), geteuid());
		ret = seteuid(0);
		if (ret != 0) {
			SPINE_LOG_DEBUG("Spine NOT able to set effective UID to 0");
		}

		if (geteuid() != 0) {
			SPINE_LOG_DEBUG("Spine NOT running as root.  This is required if using ICMP.  Please run \"chown root:root spine;chmod u+s spine\" to resolve.");
			set.icmp_avail = FALSE;
		} else {
			SPINE_LOG_DEBUG("Spine is running as root.");
			set.icmp_avail = TRUE;

			if (seteuid(getuid()) == -1) {
				SPINE_LOG_DEBUG("Spine unable to drop from root to local user.");
			}
		}
	} else {
		SPINE_LOG_DEBUG("Spine has cap_net_raw capability.");
		set.icmp_avail = TRUE;
	}
	SPINE_LOG_DEBUG("Spine has %sgot ICMP", set.icmp_avail?"":"not ");
	#endif
	#endif
}
