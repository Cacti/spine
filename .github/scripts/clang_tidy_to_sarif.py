...
def parse_positive_int(raw: str, field: str, line: str) -> int:
	try:
		...
		raise ValueError(f"invalid {field} value in clang-tidy line: {line}") from exc
	if value < 1:
		raise ValueError(f"invalid {field} value in clang-tidy line: {line}")
	return value
...
def main() -> int:
	...
	for raw_line in text.splitlines():
		...
		try:
			...
			col = parse_positive_int(m.group("col"), "column", raw_line)
		except ValueError as exc:
